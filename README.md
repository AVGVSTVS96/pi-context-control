...
Interactive context window manager for [pi](https://pi.dev). See exactly what is in your LLM context (every message, reasoning block, tool call, and tool result, with token estimates) and shape what gets sent: mask anything in or out of context at will, or swap a span of history for a compact LLM-written summary.

Everything is **non-destructive**: changes are applied via pi's `context` event right before each LLM call. The session file is never modified, and every change is reversible at any time.

## Install

```bash
pi install https://github.com/AVGVSTVS96/pi-context-control
```

Or add it to `.pi/settings.json` (project) or `~/.pi/settings.json` (global) yourself:

```jsonc
{ "packages": ["https://github.com/AVGVSTVS96/pi-context-control"] }
```

A local clone works too: `pi install ./path/to/pi-context-control`.
...
The panel renders in flow between the transcript and the input, stacking with widgets from other extensions. `i` hands the keyboard back to the editor while the panel stays visible; `/ctx` or `ctrl+alt+c` grabs it back. `esc` closes the panel. `ctrl+c`/`ctrl+d` always pass through to pi.
...
`s` (session view) **summarizes** a span instead of hiding it: `s` anchors a range, `↑`/`↓` extend it, `s` again confirms (`s` `s` takes just the selected row, `esc` cancels). The span is distilled in the background (a notice confirms it started) and one compact digest replaces it in context, at the same position. The selection decides what is kept verbatim: take the whole turn, or only its middle so the question and answer survive word-for-word.

- `enter` on a `§ summary` row opens the full digest text, with the model that wrote it; `↑`/`↓` scroll, `esc` backs out.
...
Prompt caching is prefix-based, so any change (mask, unmask, or summary) rewrites everything after it on the next call. The panel prices every action before you take it:

- The footer previews the selected node: `mask: saves ~1.2K/call · rewrites ~8.0K cached · pays off in ~16 calls`. Payback uses the active model's own cache pricing. A summarize selection is previewed the same way before you confirm: `summarize: ~2.3K across 5 items · rewrites ~8.0K cached · <s> to confirm`.
- Unapplied changes show a pending line, and batched masks break the cache once, at the earliest change: `⚡ pending: cache breaks at turn 3 · ~8.0K rewritten next call (45.2K cached now)`. The cached size is the provider's real usage number.
...
`.pi/context-control.json` (project) or `~/.pi/context-control.json` (global) takes custom presets, which appear after the built-ins in the menu, and a summarizer model override:
...
Preset fields (all conditions ANDed): `types` (leaf kinds: `tool-result`, `tool-call`, `reasoning`, `assistant-text`, `user-text`, `user-image`, `meta`; defaults to tool results), `tools` (restrict to specific tools), `olderThanTurns`, `largerThanTokens`.
...
```bash
npm install
npm test             # regression suites: masking/cache logic + panel rendering
npm run typecheck
```
